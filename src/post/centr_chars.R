...
rownames(freq.tab) <- data$char.info[,COL_SHORT_NAME]
for(c in 1:nrow(data$char.info))
{	name <- tolower(data$char.info[c, COL_SHORT_NAME])
...
{	tlog(2, "Processing volume ",v)
	...
	for(i in idx)
		tlog(4, data$char.info[i,COL_SHORT_NAME], ": ", freq.tab[i, v])
	tlog(2, "")
}
...
rownames(est.tab) <- data$char.info[,COL_SHORT_NAME]
for(v in volumes)
{	tlog(2, "Processing volume ",v)
