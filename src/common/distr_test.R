...
{	# init
	...
	if(all(data>=0) && any(data==0))	# just to avoid zeroes in the data, which prevents fitting certain distributions
	{	msg <- "Adding 1 to all values, in order to avoid zeroes";tlog(2,msg);msgs <- c(msgs, paste0("..",msg))
		data <- data + 1
	}
