...
{	tlog(2, "Processing first centrality measure: ",centr1.name)
	...
	{	tlog(4, "Processing second centrality measure ",centr2.name)
		...
		centr1.vals.unf <- load.static.nodelink.stats.scenes(weights="none", measure=centr1.name, filtered="unfiltered", compare=FALSE)
		centr1.vals.flt <- load.static.nodelink.stats.scenes(weights="none", measure=centr1.name, filtered="filtered", compare=FALSE)
		
		# get centrality 2 values
		centr2.vals.unf <- load.static.nodelink.stats.scenes(weights="none", measure=centr2.name, filtered="unfiltered", compare=FALSE)
		centr2.vals.flt <- load.static.nodelink.stats.scenes(weights="none", measure=centr2.name, filtered="filtered", compare=FALSE)
...
#		summary(fit)
		...
		plot.file <- get.path.stats.topo(mode="scenes", char.det="implicit", net.type="static", meas.name=MEAS_MULTI_NODES, weights="none", filtered="both", suf=paste0("centr_",centr2.name,"_vs_",centr1.name))
