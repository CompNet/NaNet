...
# Requires: zeta.R
# NOTE parameter name changed from to "xmin" to "threshold"
yule.fit <- function(x, threshold) {
  x <- x[x>=threshold]
  n <- length(x)
  negloglike <- function(a) { -yule.loglike(x,a,threshold) }
  # Invoke zeta estimator, in simplified discrete form, to get an initial
  # value
  a0 <- zeta.fit(x,threshold=threshold,method="ml.approx")$exponent
  est <- nlm(f=negloglike,p=a0)
  fit <- list(type="Yule", threshold=threshold, exponent=est$estimate[1],
