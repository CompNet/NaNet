...
#		vols.idx <- match(vols, volume.stats[,COL_VOLUME])
		...
		{	tlog(7,"For weight \"",w,"\"")
			...
			{	table.file <- get.path.stat.table(object="nodes", mode="scenes", char.det="implicit", net.type="static", weights=w, vol=paste0(v,"_",volume.stats[v,COL_VOLUME]), filtered=filt.txt, compare=FALSE)
