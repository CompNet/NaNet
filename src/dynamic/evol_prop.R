...
{	meas <- gr.meas[m]
	...
	# plot the measure
	plot.file <- get.path.stats.topo(mode="scenes", char.det="implicit", net.type="narr_smooth", order=ord.fold, meas.name=meas, filtered=filt.txt)
...
{	meas <- vx.meas[m]
	...
	{	# compute y range
		...
		# plot the measure
		plot.file <- get.path.stats.topo(mode="scenes", char.det="implicit", net.type="narr_smooth", order=ord.fold, meas.name=meas, filtered=filt.txt, suf=pt)
		tlog(6, "Plotting in file \"",plot.file,"\"")
		for(fformat in PLOT_FORMAT)
		{	if(fformat==PLOT_FORMAT_PDF)
				pdf(file=paste0(plot.file,PLOT_FORMAT_PDF), bg="white", width=15, height=5)
